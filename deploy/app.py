...
def predict_with_validation(input_method, uploaded_image, drawn_image):
    ...
    try:
        ...
        try:
            ...
            # For drawn images, handle RGBA from the canvas properly
            if input_type == "drawn":
                img_array = np.array(image)
                
                # Handle RGBA properly - composite onto white using alpha channel
                if len(img_array.shape) == 3 and img_array.shape[2] == 4:
                    alpha = img_array[:, :, 3].astype('float32') / 255.0
                    rgb_grey = np.mean(img_array[:, :, :3], axis=2)
                    img_array = (rgb_grey * alpha + 255.0 * (1.0 - alpha)).astype('uint8')
                elif len(img_array.shape) == 3:
                    img_array = np.mean(img_array[:, :, :3], axis=2).astype('uint8')
...
def predict_with_preview(image):
    ...
    if image is None:
        # None for image components, error text for the third -- same tuple pattern
